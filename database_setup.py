...
def create_table(cur: sqlite3.Cursor):
    cur.execute(("""CREATE TABLE IF NOT EXISTS user_records(claimed_email PRIMARY KEY, fname, lname, title, dept)"""))
    cur.execute("""CREATE TABLE IF NOT EXISTS responses(entryNum PRIMARY KEY , prefix, fName, lName, title, orgName,
            ...
            claimed_email DEFAULT '', FOREIGN KEY(claimed_email) REFERENCES user_records(claimed_email))""")
